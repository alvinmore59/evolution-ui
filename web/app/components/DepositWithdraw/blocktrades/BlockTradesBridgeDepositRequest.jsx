...
class BlockTradesBridgeDepositRequest extends React.Component {
	...
	urlConnection(checkUrl, state_coin_info)
	{
        ...
        .then((json_responses) => {
            ...
            let allowed_mappings_for_deposit = {}; // all non-evo to evo
            let allowed_mappings_for_withdraw = {}; // all evo to non-evo
			let allowed_mappings_for_conversion = {}; // all evo to evo
            trading_pairs.forEach(pair => {
                    ...
                    output_coin_info.backingCoinType != pair.inputCoinType && (input_coin_info.restricted == false) && (output_coin_info.restricted == false))
                {
                    ...
                    {
                        if (input_coin_info.walletType != "evolution2" &&
                            output_coin_info.walletType == "evolution2")
                        {
                        ...
                        else if (input_coin_info.walletType == "evolution2" &&
                                 output_coin_info.walletType != "evolution2")
                        {
