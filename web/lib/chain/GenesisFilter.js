import {hash, key} from "evolution-js/es";

var evo_genesiskeys_bloom_url = undefined
try {
    var url = require("file-loader?name=evo_genesiskeys_bloom_[sha1:hash:hex:7].dat!assets/evo_genesiskeys_bloom.dat")
    if(url.indexOf("3cee441") === -1)
        throw new Error("Incorrect hash: evo_genesiskeys_bloom.dat")
    evo_genesiskeys_bloom_url = url
} catch(e) {
    // webpack deployment exception (not run time)
    console.log("WARN: Will be unable to filter EVO 1.0 wallet imports, did not find assets/evo_genesiskeys_bloom.dat", e)
}

/**
    This should only be applied to a EVO 1.0 export file taken on the
    discontinued chain. Any public key string or address (all 5 formats) carried
    over to the EVO 2.0 genesis block will be in this filter.
...
export default class GenesisFilter {
    ...
    isAvailable() { return evo_genesiskeys_bloom_url !== undefined }
    ...
    init(done) {
        ...
        xhr.open("GET", evo_genesiskeys_bloom_url)
        xhr.send()
    }
    ...
    filter( account_keys, status ) {
        if( ! this.isAvailable() ) {
            console.log("WARN: Missing bloom filter for EVO 0.9.x wallets")
            ...
            try {
                ...
                for(var a = 0; a < account_keys.length; a++) {
                    ...
                    for(var k = keys.encrypted_private_keys.length - 1; k >= 0; k--) {
                        ...
                        if( /^EVO/.test(currentKey) ) currentKey = "EVO" + currentKey.substring(3)
                        if(this.inGenesis( currentKey )) continue
                        var addresses = key.addresses(currentKey, 'EVO')
