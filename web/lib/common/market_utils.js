...
import {ChainStore, ChainTypes} from "evolution-js/es";
...
const MarketUtils = {
    ...
    parseOrder(order, base, quote, invert = false) {
        ...
        if (order.debt) {
            if (invert) {
                // Price in USD/EVO, amount should be in EVO, value should be in USD, debt is in USD
                // buy is in USD, sell is in EVO
                // quote is USD, base is EVO
                value = order.debt / quotePrecision;
                amount = this.limitByPrecision(value / price.full, base);
            } else {
                // Price in EVO/USD, amount should be in USD, value should be in EVO, debt is in USD
                // buy is in USD, sell is in EVO
                // quote is USD, base is EVO
